...
// The hook created will be asynchronous, and it's the responsibility of the application to call the Flush method
...
func (hook *GraylogHook) sendEntry(entry graylogEntry) {
	...
	m := Message{
		...
		Level:     level,
		LevelName: logrusLevelToString(entry.Level),
		File:      entry.file,
		Line:      entry.line,
		Extra:     extra,
	}
