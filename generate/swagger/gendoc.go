...
func renderReplyAsDefinition(d swaggerDefinitionsObject, m messageMap, p []spec.Type, refs refMap) {
	for _, i2 := range p {
		...
		if !hasRequestParameters(refs, defineStruct.RawName) { //response需要添加自定义结构
		...
		var properties *swaggerSchemaObjectProperties
		for _, member := range defineStruct.Members {
			...
			if kv.Key == "" {
				...
				for _, m := range memberStruct.Members {
					...
					*properties = append(*properties, mkv)
				}
				continue
			}
			if properties == nil {
...
func hasRequestParameters(requestResponseRefs refMap, reqName string) bool {
	_, ok := requestResponseRefs[fmt.Sprintf("#/definitions/%s", reqName)]
	return ok
}
