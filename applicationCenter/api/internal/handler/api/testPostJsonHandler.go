...
func TestPostJsonHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.TestPostJsonReq
		if err := httpx.Parse(r, &req); err != nil {
			httpx.Error(w, err)
			return
		}

		l := api.NewTestPostJsonLogic(r.Context(), svcCtx)
		err := l.TestPostJson(&req)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.Ok(w)
